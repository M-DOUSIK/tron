...
/* Session 11 fix (MedSight): the vendored default here (196) assumes one
 * more interrupt vector than this project's actual, ST-generated
 * Application/Startup/startup_stm32n657x0hxq_fsbl.s provides. That file's
 * .isr_vector table is verified (via `size -A` on the linked .elf) to be
 * exactly 844 bytes = 211 words total = N_SYSVEC(16) + 195 real IRQ slots,
 * ending at LTDC_UP_ERR_IRQHandler. Left at 196, knl_start_mtkernel()'s
 * ROM->RAM vector-table copy loop (sys_start.c) reads one word past the end
 * of our real table into whatever flash happens to follow it, and copies
 * that garbage into knl_exctbl[]'s last slot. */
#define N_INTVEC		195	/* Number of Interrupt vectors */
