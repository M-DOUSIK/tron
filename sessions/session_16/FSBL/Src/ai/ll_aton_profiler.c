                            ...
                            int groups, char *conv_name)
{
  ...
  for (int n = 0; n < N; ++n) // input batch
  {
    for (int k = 0; k < K; ++k) // num kernels per group
    {
      ...
      /* %ld, not %d. maxmax_k is int32_t, which on this ABI is `long int`,
       * so %d is the wrong specifier and GCC has said so for several
       * sessions. The cast makes the promotion explicit rather than relying
       * on int32_t and long happening to agree. */
      LL_ATON_PROFILER_PRINTF("%s k=%d %ld scale=%g\n", conv_name, k,
                              (long)maxmax_k, (double)kern->scale[k]);
    }
  }
...
#define DIFFTH 1
  ...
  if (f)
  {
    /* %ld for the same reason: maxmax is int32_t == `long int` here. */
    LL_ATON_PROFILER_FPRINTF(f, "%s %ld\n", conv_name, (long)maxmax);
    /* was: log((float)maxmax) / log(2.0) */
    LL_ATON_FCLOSE(f);
  }
#endif
