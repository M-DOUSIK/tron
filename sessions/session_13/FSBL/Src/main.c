 ...
 * appeared.
 *
 * SESSION 13 UPDATE. GUI_BUFFER_ADDRESS (0x342BB800, main.h) is the new
 * second/UI framebuffer added this session — it sits directly above
 * BUFFER_ADDRESS + FRAME_BUFFER_SIZE, inside the same AXISRAM3-6 span the
 * MEMLPENR bits above already ungate. No new LPEN bit was needed for it; the
 * rule above ("any new DMA-driven peripheral must get its LPEN bit added
 * here") did not trigger because nothing new moves data into it — it is
 * drawn CPU-side exactly like BUFFER_ADDRESS always was. If a future session
 * places any buffer OUTSIDE AXISRAM3-6, or adds a DMA path that writes to
 * one (a second LTDC layer, a DMA2D destination elsewhere), re-derive this
 * table from scratch and re-test from a cold boot — do not assume coverage.
 * ════════════════════════════════════════════════════════════════════════ */
