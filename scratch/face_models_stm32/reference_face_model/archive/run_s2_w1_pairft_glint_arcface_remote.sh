...
# Repository checkout on the training box.
cd "${REPO_ROOT:?set REPO_ROOT to the repository checkout}"

# Optional: export HTTP_PROXY / HTTPS_PROXY / ALL_PROXY if the training box
# needs a proxy to reach the dataset mirrors. Inherited from the environment.
...

{
    ...
    if [[ "$(current_images)" -lt 200000 ]]; then
        uv run python -u download_glint360k_subset.py \
